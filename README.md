# 개리그 온라인 2026 (KLO26)

**K리그 2026 29개 구단 1,056명으로 내 스쿼드를 짜서, 브라우저에서 P2P 로 1:1 실시간 조작 축구 대전.**
설치·가입·서버 없음. PC 전용. 비상업.

> 🚧 **개발 중 (2026-09-09).** 스쿼드를 짜서 **봇과도, 다른 사람과도** 한 판 할 수 있습니다.
> 오프사이드·파울·카드·PK·교체까지 들어갔고 심판도 뜁니다.
> K리그 2026 **29개 구단 1,056명**이 들어가 있습니다 (원작자 허락 확인 완료).
> 선수 명단·능력치는 **개리그 매니저(KM26)** 원본을 따라갑니다 — 2026-09-09 갱신분 반영.

- 배포: <https://goormigrm.github.io/KLO26/> — 스쿼드를 짜고 **혼자 하기** 또는 **방 만들기**
- 저장소: <https://github.com/goormigrm/KLO26>

## 이런 게임이 됩니다

- **카드 1,056장 전원 개방** — 뽑기도 이적시장도 없습니다. 대신 **급여 상한** 안에서 골라야 합니다.
- **구단을 고르면 그 구단 선수로 스쿼드가 채워집니다** — 전술판에서 자리를 끌어다 옮기고 선수를 바꿉니다.
- **스쿼드 수정** — 구단을 다시 고르지 않고 선수만, 자리만, 선발↔후보만 바꿀 수 있습니다.
  자리를 고르면 바꿀 수 있는 자리에 초록 테두리가 뜨고, 선발↔후보는 버튼 한 번입니다.
- **팀워크** — 선발 11명 중 같은 구단이 5·7·9·11명이면 전원 능력치 +1·2·3·4.
  전력이 낮은 구단이면 **약체 가산**이 최대 +3 더 붙습니다 (순수 구단 팀 최대 +7).
- **타 구단 영입은 선발 5명·후보 2명까지** — 다른 구단 선수는 급여가 **1.5배** 듭니다.
  제한이 없으면 어느 구단을 골라도 같은 올스타 팀이 되어 버립니다.
- **강화는 예산제** — 스쿼드마다 24 포인트, 카드당 +5 까지. 확률 없음.
- **직접 조작** — 공을 가진 선수를 키보드로. 키 배치는 **축구 게임에서 익숙한 표준 키보드 배치 그대로**(방향키 이동 · S 패스 · W 스루 · A 로빙 · D 슛 · E 대시, 수비 때 S 선수 변경 · A 슬라이딩 · D 압박 · Space 태클 · Q 팀 압박 · C 견제 — 그리고 Z·Q·C·F 조합, 녹온·딩크·페이크·파워 슛, 골키퍼 직접 조작까지). 나머지 20명은 AI.
- **🎯 조작 연습** — 선수 한 명으로 키 하나를 한 상황에서 성공할 때까지. 25가지.
- **PC 키보드 전용** — 폰에서는 안내만 뜨고, **게임패드는 지원하지 않습니다**(계획에 없습니다). 페이스 컨트롤은 **Shift** 입니다(Ctrl+W 가 브라우저 탭을 닫아서).
- **방송 카메라 · 찰흙 선수** — Three.js. 선수 키가 그대로 보이고, 생김새는 닮게 그리지 않습니다.
- **선수 이름은 보호명** — 실명이 아니라 성씨를 바꾼 이름입니다(김→구, 이→여 …). 외국 선수는 첫 글자 초성을 밉니다.
- **구단명은 가상 이름** — 개리그 매니저와 같은 이름을 씁니다 (안양 치토스 · 울산 오션 FC · 전북 모터스 …). 지역명만 실제와 같고 나머지는 지어낸 것입니다. 유니폼은 구단 색 하나로 만듭니다.
- **전후반 3분씩** 실시간(경기 시계 ×15) + **추가시간**(죽어 있던 시간만큼 1~5분, 공이 죽었을 때 휘슬). 오프사이드·파울·경고/퇴장·프리킥·PK·**교체 5명(기회 3번, 전술판)**·백패스 룰. 직접 프리킥·PK 는 ← → 코너 · ↑ ↓ 높이로 조준하고 예상 궤적이 보입니다. **심판 3명**이 뜁니다. 킥오프는 아군에게 짧은 패스로만.
- **조작 보조** — 방향키를 누르면 조작 선수 앞에 `›` 가 방향을 보여 주고, 슛은 언제나 골문을 겨눕니다(위/아래 키로 코너). 수비 때 D 를 누르고 있으면 선수가 공을 향해 스스로 달립니다.
- **스쿼드 화면** — 맨 위 저장 슬롯 5칸, 급여는 큰 숫자와 "남은 급여"로, 선수마다 "바꾸면 ±얼마"가 보입니다. 강화는 상자에 설명과 자동 배분이 있습니다. **자동 채우기는 선발/후보 따로**, 기준은 포메이션 능숙도 우선 / 능력치 우선. 스쿼드는 **JSON 파일**로 저장·불러오기. 저장 안 하고 돌아가기도 됩니다.
- **능력치는 별로만** — OVR·세부 능력치는 정확한 숫자 대신 ★ 5개(반 개 단위)로 대략만 보입니다. 급여와 자리 능숙도는 숫자입니다(규칙이라서).
- **유니폼은 단색** — 홈은 구단 색 하나, 원정은 골키퍼만 빼고 전신 흰색입니다.
- **방을 만들고 P2P 로** — 서버가 없어도 두 브라우저가 같은 경기를 봅니다(결정론 락스텝).
- **혼자 하기** — 봇 3단계. 상대는 실제 구단입니다.
- **소리도 코드로** — 관중석·휘슬·킥·골·로비 배경음까지 전부 Web Audio 절차 생성입니다. 음원 파일이 없습니다.

## 문서

| 문서 | 내용 |
|---|---|
| [docs/플레이-가이드.md](docs/플레이-가이드.md) | **처음 하는 사람에게 그대로 보여 줄 문서** — 조작 · 스쿼드 · 온라인 · FAQ |
| [docs/DESIGN.md](docs/DESIGN.md) | **설계서 정본** — 규칙·조작·시뮬·카드·네트워크·화면·개발 단계·검증 기준 |
| [docs/DECISIONS.md](docs/DECISIONS.md) | 갈림길과 근거 — 사용자 결정 3건 · 기본안 17건 · 확인 필요 5건 |
| [docs/PREP.md](docs/PREP.md) | 사용자가 직접 해야 하는 것 (허락 재확인 · 저장소 · Pages · 기기) |
| [HANDOVER.md](HANDOVER.md) · 개선 계획은 [docs/개발계획-FC온라인-벤치마크.md](docs/개발계획-FC온라인-벤치마크.md) | 개발 인수인계 **정본** — 현재 상태, 다음 할 일, 작업 규칙 |
| [CHANGELOG.md](CHANGELOG.md) | 변경 이력 |
| [docs/면접-질문답변.md](docs/면접-질문답변.md) | **개발자 면접용** — 기술 스택·아키텍처·결정론/P2P·밸런스 방법론·대표 버그 사례를 질문·답변으로 |
| [docs/공지글-모음.md](docs/공지글-모음.md) | 공개 때 붙여 넣을 공지 문안 — 1차 오픈 베타(패치노트 꼴) · 짧은 문안 · 댓글 답변 · 2차 틀 · 첨부 목록 |

## 개발

필요한 것: **Node.js 20 이상**(24 권장), **git**.

```bash
git clone https://github.com/goormigrm/KLO26.git
cd KLO26
npm install
npm run dev     # http://localhost:5175/KLO26/
```

| 명령 | 하는 일 |
|---|---|
| `npm run dev` | 개발 서버 (HMR) |
| `npm test` | vitest — 결정론 · 성능 · 경기 · 규칙 · 스쿼드/코드 · 키 매핑 · 카메라 (64개) |
| `npm run build` | `tsc --noEmit` + vite build → `dist/` |
| `npm run balance -- 50` | 봇 vs 봇 50판 계측 (골 분포 · 슛 · 점유 · 패스 성공률) |
| `npm run probe -- 101` | 시드 101 한 경기 진단 (사건 · 공 위치 분포) |
| `npm run verify` | 단계 7 검증 — 슬라이더 지문 · 짝 비교 · 팀워크/강화 효과 · 홈원정 대칭 · 결정론 |
| `npm run economy` | 급여·영입 경제 진단 — 구단별 최강 선발과 격차 |
| `npm run power` | 구단 전력(최고 18명 급여 합) — 약체 가산 구간 확인 |
| `npm run attrcheck` | 능력치 묶음별 A/B — 능력치가 경기에 닿는지 (대조군 50%) |
| `npm run tactics` | 전술 프리셋별 라인 높이·퍼짐·결과 — 전술이 움직임을 바꾸는지. `-- 16 "이름:line=3,…"` 로 임의 프리셋도 |
| `npm run shots` | 자리(거리·각도·결정력·달리기·힘)별 슛 전환율 — 골키퍼가 얼마나 막는지 |
| `npm run stamina` | 하프타임·종료 시점 체력 분포 — 경기 길이에 맞게 닳는지 |
| `npm run throwin` | 스로인 뒤 5초 안에 누가 잡는지 (가로채기 비율) |
| `npm run oneone` | 1대1 (공격수 vs 골키퍼) 전환율 |
| `npm run asym 200` | 홈/원정 대칭 — 같은 스쿼드끼리 N 판, 하프별 골·슛·점유 (공개 전 필수, 목표 47~53%) |
| `npm run enhcheck 200 even 1` | 강화가 값을 하나 — 배분(`even`·`att`·`def`·`tw4` 기준선)·단계별 짝 비교 A/B. 목표 +8~15%p |
| `npm run balance -- 120 180 swap` | 두 스쿼드 자리를 바꿔 돈다 — "홈 승 : 원정 승" 이 홈 이점인지 스쿼드 전력 차인지 가른다 |
| `npm run asymprobe 400 base 10000` | 대칭이 깨졌을 때 원인 가르기 — `swapdir`(진영) · `ko1`(킥오프) · `both` 변형과 하프별 전체 통계 |
| `npm run cards` | 카드 OVR 보정 · 급여 상한 계산 |
| `npm run clubs 90` | 실제 구단끼리 90판 계측 (1부/2부 차이 · 골) |
| `npm run build:cards` | 원본 명단 → `src/data/cards.json` 다시 굽기 (원본은 저장소 밖) — 원본이 갱신됐을 때의 전체 절차는 [HANDOVER.md](HANDOVER.md) "원본 명단 갱신" |

**테스트 모드**: 주소 뒤에 `?test=1` 을 붙이면 로비에 테스트 타일이 생깁니다 — **AI 대 AI 관전**과 **키 입력 표시**.
배포에서는 링크를 걸지 않으므로 보이지 않습니다.

`main` 에 push 하면 GitHub Actions 가 테스트·빌드 후 GitHub Pages 로 배포합니다.
커밋 작성자는 저장소마다 설정합니다: `git config user.name goormigrm` / `git config user.email 1117tkdrms@gmail.com`

### Claude Code 로 이어서 개발하기

저장소 폴더를 열고 **"HANDOVER.md 읽고 이어서 진행해줘"** 한 줄이면 됩니다.

### 구조 (설계서 8장 · ✅ = 있음)

```
src/core/     ✅ 결정론 시뮬레이션 (물리·규칙·AI·포메이션·스킬·합성 스쿼드) — 렌더/DOM 금지
src/cards/    ✅ 카드 6스탯·OVR·급여·스쿼드 규칙·스쿼드 코드
src/data/     ✅ 카드 1,056장 (`cards.json` — build_cards.py 가 굽는다. 보호명)
src/net/      ✅ Trystero 로비·방, 2인 락스텝
src/game/     ✅ 혼자 하기 세션 루프 · 키보드 입력 · Worker 틱 타이머
src/render3d/ ✅ Three.js 방송 카메라 · 찰흙 선수(키 반영·동작) · 피치·골대·관중석 · 공
src/render/   ✅ HUD(DOM 오버레이) · 레이더
src/audio/    ✅ 효과음·배경음 (Web Audio 절차 생성 — 음원 파일 없음)
src/ui/       ✅ 로비 · 스쿼드 · 대기실 · 설정 · 폰 차단
tools/        ✅ 밸런스 계측 · 한 경기 진단 (카드 빌드·캘리브레이션은 단계 1)
tests/        ✅ vitest
```

**핵심 규칙**: `src/core/` 에서는 `Math.random`·삼각함수·시간 함수를 쓰지 않습니다. 두 브라우저가 같은 입력으로 같은 결과를 내야 하기 때문입니다. [docs/DESIGN.md](docs/DESIGN.md) 4.12.

## 만드는 것

TypeScript · Vite · Trystero(WebRTC) · Three.js · GitHub Pages. 그래픽·소리는 전부 코드로 생성하고 외부 에셋·3D 모델을 쓰지 않습니다.

**비용 원칙**: 카드 등록이 필요한 서비스는 무료 티어라도 쓰지 않습니다(TURN 배제). GitHub Pages·Google Fonts·공개 릴레이·무료 STUN 만 씁니다.
