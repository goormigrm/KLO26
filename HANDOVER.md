...
> **9/23 (32차, 메인 PC)**: 0-c 로드맵 잔여 둘 — **⚽ 드리블 터치 간격**(공을 밀고 따라간다 · 소유는 그대로 · 뺏기는 틈에서 · A/B 유지, G-75) · **🎥 리플레이 측면 컷 눈으로 확인**(카메라 좌표 + 캡처) — 와 재검증 WARN **크로스 때 박스가 빈다**(아래 32차 절).
...
| 테스트 | `npm test` — **31 파일 · 225개** 전부 통과 (skip 0 · 31차에 `fc_controls` 24 · `practice` 10). 계측을 같이 돌리는 중엔 5초 기본 타임아웃에 걸리는 테스트가 있어 긴 것들은 60초를 줬다 |
