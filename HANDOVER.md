...
| 테스트 | `npm test` — **30 파일 · 218개** 전부 통과 (skip 0 · 31차에 `fc_controls` 24 · `practice` 10). 계측을 같이 돌리는 중엔 5초 기본 타임아웃에 걸리는 테스트가 있어 긴 것들은 60초를 줬다 |
...
**다음에 볼 것 (31차에서 찾고 안 고친 것)** — **골키퍼가 먼 슛에 너무 일찍 몸을 날린다**: `gkCatch` 가 반응 시간만 지나면 손이 안 닿는 공에 곧바로 다이브(24틱) → 공이 오기 전에 넘어져 0.5 초 → 굴러오는 공이 지나간다. 20 m 보통 슛 25%, 가까운 포스트 감아차기 64%(`tests` 의 A/B 스크립트 방식으로 쟀다). 공이 오기 0.3~0.45 초 전까지는 옆걸음으로 따라가게 하면 되지만 **봇 경기 득점이 크게 바뀐다** — asym 500 · audit 60 A/B 로 재고 넣을 것.
