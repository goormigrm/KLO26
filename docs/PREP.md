...
- [x] **A-1 KM26 원작자에게 허락 재확인** — **받았다 (2026-09-09, 사용자 확인).** 이에 따라 `tools/build_cards.py` 로 실제 명단을 구워 넣었다.
  선수는 **보호명**(성씨 치환·외국인 초성 시프트·충돌 회피), 구단은 **지역명만 남기고 기업명을 뺐다**. 저장소에 실명 0건은 그대로다.
  원래 문구: KMD26 이 받은 허락(매치엔진·선수/포메이션 데이터·CSS·UI)은 KMD26 에 대한 것이다.
