...
> 남은 것은 **다른 회선 두 대 실제 대전**(PREP B-1), **강화 효과 키우기**, 2차 피드백, 그리고 공개입니다.
...
| 테스트 | `npm test` — **26 파일 · 167개** 전부 통과 (skip 0). 계측을 같이 돌리는 중엔 5초 기본 타임아웃에 걸리는 테스트가 있어 긴 것들은 60초를 줬다 |
...
| 0-e | ✅ **홈/원정 기울기 — 29차에서 닫음** (G-54). 진영·킥오프가 아니라 `step` 의 팀 번호 순서 셋. 2,000판 47.1% → **50.5%**(골 1.91 · 1.91). 공개 전 `npm run asym 500` 한 번 더 | `asym 500` 이 47~53% |
