...
> **9/23 (31차 넷째)**: 제보 둘 — **다이브 반대쪽 공을 막는 건 그림이었다**(sim 600번에 0번 · 렌더가 옆 속도 0 이면 지난 다이브 쪽으로 눕혔다 → 시작 때 한 번, 없으면 공 쪽) · **PK·프리킥 방향**은 D 와 방향키를 같이 떼면 방향이 사라졌고(0.33 초 기억 `kickStick`) 좌우 없으면 아무 구석이었다(→ 가운데). 파넨카 연습 골키퍼 = 사람 PK 다이빙과 같은 함수(G-74). 봇 골 2.75 · 대칭 50.3%.
> **9/23 (31차 셋째)**: **🧤 골키퍼 조기 다이브 고침** — 먼 슛엔 옆걸음으로 따라가다 공이 올 때 난다(G-73). 봇 골 3.67 → 2.78 · 대칭 50.8%.
> **9/23 (31차 둘째)**: **🤖 조작 연습 AI 시범**(성공하는 시드를 먼저 찾아 같은 입력으로 보여 준다 · 누르는 키에 불) → 가운데 설명 → Enter · 시범을 맞추다 **엔진 구멍 여섯** 고침(크로스 받는 선수가 공 밑으로 가던 것 · 벽 위를 넘은 프리킥을 벽이 헤딩 · 슬라이딩 겨냥 …, G-72). 봇 골 3.15 → 3.67 · 대칭 500판 48.8%.
> **9/23 (31차, 노트북)**: **🎮 FC 온라인 조작 전부**(나무위키 키보드 조작법 대조 — 못 넣은 둘은 G-67) · **🎯 조작 연습 모드 25가지** · **채팅은 Enter**(세레모니 중만 건너뛰기, G-65) · 규칙·물리 고침(벽이 프리킥을 막는다 · 칩이 키퍼를 넘는다 · 키퍼 손은 박스 안 · 상대 둘 미만이면 오프사이드 없음). 입력 8바이트 · `APP_ID klo26-v2`. 대칭 500판 **52.2%**.
> **9/23 (32차, 메인 PC)**: 0-c 로드맵 잔여 둘 — **⚽ 드리블 터치 간격**(공을 밀고 따라간다 · 소유는 그대로 · 뺏기는 틈에서 · A/B 유지, G-75) · **🎥 리플레이 측면 컷 눈으로 확인**(카메라 좌표 + 캡처) — 와 재검증 WARN **🎯 크로스 때 박스가 빈다**(날아가는 동안 박스로 계속 · 머리 높이로 · 크로서가 기다린다, G-76 — 아래 32차 절).
> 남은 것은 **다른 회선 두 대 실제 대전**(PREP B-1 — 이때 채팅·새 조작도 본다), 2차 피드백(새 조작의 손맛 · 연습 모드), 그리고 공개입니다.

## 세션 메타데이터

| 항목 | 값 |
|---|---|
| 날짜 | 2026-09-23 (32차 메인 PC — 직전 30·31차: 같은 날 노트북 / 28·29차: 09-16 노트북 / 14~27차: 09-11~15 메인 PC) |
| 저장소 · 브랜치 | `https://github.com/goormigrm/KLO26` · `main` |
| 마지막 커밋 | 32차 (`git log -1`). 그 앞 `0d5a2b0` 31차 넷째 · `a55d234` 31차 · `10cb457` 30차 |
...
| 테스트 | `npm test` — **32 파일 · 227개** 전부 통과 (skip 0 · 31차에 `fc_controls` 24 · `practice` 10 · 32차에 `cross` 2). 계측을 같이 돌리는 중엔 5초 기본 타임아웃에 걸리는 테스트가 있어 긴 것들은 60초를 줬다 |
...
## 31차 (2026-09-23, 노트북) — FC 온라인 조작 · 조작 연습 · 채팅 Enter

정본: DESIGN **3.1 표 · 3.1a · 3.1b** · DECISIONS **G-65 ~ G-70** · 플레이 가이드 **2장**(2-1 ~ 2-5).

| 무엇 | 구조 | 다음 사람이 알아야 할 것 |
|---|---|---|
| 입력 | `core/input.ts` — buttons **32비트**, `INPUT_BYTES 8`, 새 비트 F·CTRL·GK(\`)·TAC1~4, 숫자 1~0 은 21~24번 **4비트 값**(`presetPick`). `[ ]` 는 이제 `BTN_BAL_UP/DOWN`(공수 밸런스) | 규격이 바뀌면 `net/room.ts APP_ID` 를 올린다(이번 v2) |
| 사람 조작 | `core/sim.ts handleInput` 한 곳 — 공격 · 골키퍼 공 · 수비 · 세트피스 수비(벽·PK 골키퍼) · 리스타트 킥커(PK 조합키·Shift 자리) · 파워 슛 준비 · "한 번 더" | **봇은 이 경로를 안 탄다.** 새 동작의 난수는 사람 입력에서만 — 봇 대 봇 해시가 그대로다 |
| 한 번 더 | `ball.followTap` — 공이 떠난 뒤 `TAP_TICKS`(9) 안의 같은 키가 날아가는 공을 바꾼다(딩크·낮은 크로스·드리븐·슈퍼 녹온) | S 두 번째는 선수 변경으로 새지 않게 막았다. 누가 찼는지는 `team.tapBy` |
| 회전 | `ball.curl`(옆, 진행 방향 왼쪽 +) · `ball.dip`(톱스핀). `physics.applyCurl` · 누가 만지면 `clearSpin` | 새로 공을 만지는 곳을 만들면 `clearSpin` 을 부른다 |
| 벽 | `ball.wallBlock` — 직접 프리킥(`mods.freekick`)만. 6~11.5 m 의 수비수 · 어깨 0.42 m · 선 벽 머리 높이 · 뛴 벽(`team.wallJumpT` 4~40틱) 0.45 ~ 머리+0.55 | 봇 키커도 벽 위(lift 0.75)로 — 벽 판정이 생기며 예전 낮은 봇 프리킥은 다 맞았다 |
| 조작 연습 | `game/drills.ts`(상황 + 판정, DOM 없음) · `game/practice.ts`(화면) · 로비 타일 · `main.ts showPractice` | 안 쓰는 선수는 `sentOff` 로 치운다 — sim·렌더가 이미 뺀다. 슛 종류는 `ball.passKind`(해시 밖) |

**잰 것**: 대칭 `asym 500` **52.2%**(215승 88무 197패 · 골 1.90 : 1.84 — 500판 1σ ≈ 2.5%p 안). `verify` 슬라이더 지문 7/7 연결 · 짝 비교 ✅ 5 · △ 1(압박 → 상대 패스 성공률 9/16) · ❌ 1(폭 → 코너킥 2/5, 동률 11). 바꾸기 전 커밋을 worktree 에서 같은 시드로 돌리면 8/16 △ · 3/5 △ — 원래 약한 지표의 한 쌍 차이다. 팀워크 +4 효과 +12.8%p 는 30차와 같다.

**31차 둘째 — AI 시범** (G-71 · G-72): `game/demos.ts` 연습마다 대본(8방향 방향키 · 버튼 비트만) · `findDemoSeed`/`runDemo` 화면 없이 성공 시드 찾기 · `game/practice.ts` 모드 기계(list → search → demo → brief → play → success). 시범 대본이 성공하지 못하는 연습은 대개 **대본이 아니라 엔진·상황 문제**였다 — 헤딩(받을 선수가 공 밑으로) · 프리킥(벽 헤딩) · 슬라이딩(겨냥) · 로빙 스루(러너 앞질러 겨냥) · 원투(멈춘 채 시작) · 페이크(서서 시작).

**31차 셋째 — 골키퍼 조기 다이브 고침** (G-73): `gkCatch` 가 need(몸이 경로에 닿는 시간) + 0.15 초보다 공이 멀면 경로 쪽 옆걸음, 그 뒤에 난다 · `gkDecide` 는 우리 골문으로 오는 슛이 날아가는 동안 자리를 안 건드린다 · 감아찬 공 회전을 다 읽는다. 유효슛 중 골 20 m 44~55% → 24~39% · 25 m → 14~20% · 봇 판당 골 3.67 → **2.78** · 헤딩 3.0 · 대칭 500판 50.8%. 잰 방법: `tools/shots.ts 150` 을 바꾸기 전 커밋 worktree 와 나란히 · `audit 60` 같은 시드 A/B.

**31차 교훈** (아래 "주의사항 & 교훈" 에도)
- **"같은 키 두 번"은 킥이 떼는 순간 나가는 게임에선 날아가는 공을 바꾸는 것으로** — 기다렸다 차면 모든 패스가 늦어진다. 9틱 안이면 공이 1~2 m 앞이라 한 동작으로 보인다. 이 방식으로 옛 A A(첫 A 에 공이 이미 떠나 두 번째가 안 먹던 것)도 풀렸다.
- **새 조작이 뜻을 가지려면 규칙부터 본다** — 벽 점프를 넣으려 보니 벽이 슛을 막은 적이 없었고, 칩을 연습시키려 보니 칩이 키퍼를 넘은 적이 없었다. 조작만 넣었으면 눌러도 아무 일도 안 일어났다.
- **선수를 치운 상황은 11 대 11 이 숨기던 구멍을 드러낸다** — 상대가 한 명이면 오프사이드 라인이 −99 라 모든 패스가 오프사이드였다("수비수 순간이동"은 프리킥 배치였다). 퇴장 선수는 PK 때 피치로 끌려왔다(레드카드 뒤 PK 면 실제 경기에서도).

## 32차 (2026-09-23, 메인 PC) — 드리블 터치 · 리플레이 측면 컷 · 크로스

정본: CHANGELOG 9/23 두 항목 · DESIGN **4.4 · 4.10** · DECISIONS **G-75 · G-76**.

| 무엇 | 구조 | 다음 사람이 알아야 할 것 |
|---|---|---|
| 드리블 터치 | `ball.attachBall` — `Player.dribX/dribY/touchT/touchLen/touchI` · `touchGap` 을 `contestBall` 이 본다 | **소유는 끊지 않고 공 속도도 선수 속도 그대로.** 공 속도에 터치 속도를 더하면 슬라이딩 겨냥·요격·시범이 흔들린다(슬라이딩 시범 31 → 0/40 이었다) |
| 크로스 | `ai.boxRun`(박스 네 자리 — 공 가진 때 · 우리 크로스가 날 때) · `carrierDecide`(달려드는 동료에게 highcross · 크로서 기다리기 `Player.crossT`) | 크로스 가산점을 올리면 확실한 패스를 밀어내 골이 준다(+0.2 에서 4-4-2 골 1.36 → 1.00). 봇은 여전히 스루 패스 위주(판당 100번 남짓) |
| 시범 회귀 | `tools/demoall.ts` — 시범 25종 × 두 스쿼드 × 시드 40 | **엔진(ball·ai)을 바꾸면 먼저 이걸 돌린다.** 테스트는 첫 실패에서 멈춰 나머지를 가린다 |

**잰 것**: 드리블 터치 — 봇 30판 골 2.77 → 2.93 · 패스 51% 그대로 · A/B 드리블 84.2 → 83.3 · 속도 77.1 → 80.4 · 수비 87.5 → 85.0(각 120경기, 1σ 3.7). 크로스 — audit 12판 크로스 1초 뒤 박스 안 0.20 → 1.33 · 2초 안 슛 33% · 골 3.42 · 대칭 200판 51.0%. 리플레이 측면 컷 — 카메라 좌표와 `__klo.snap` 캡처로 확인(`docs/img/replay_behind.png` · `replay_side2.png`, 저장소 밖).

**32차 교훈**
- **공 "속도"는 예측에 쓰인다** — 렌더용으로 보이는 값이라도 sim 안의 다른 코드가 `b.vx` 로 앞날을 예측한다. 보이는 것만 바꾸려면 위치만 바꾼다.
- **지표가 안 움직이면 조건별로 센다** — "크로서 기다리기"를 넣고도 그대로였던 이유는 `crosswhy`(압박 60% · 몰고 들어온 윙어는 gotT 가 옛것)와 `crossflight`(공이 뜨면 모두 자리로)가 보여 줬다. 넣은 코드가 몇 번 걸리는지부터 센다.
- **시범이 깨지면 엔진보다 대본을 먼저 의심하되, 대본이 칼날 타이밍에 기대고 있었는지 본다** — 페이크 시범은 수비수를 지나치기 **전에** 차는 대본이었고, 붙은 공이 0.1 m 앞에 있어야만 빠져나갔다.
...
| 0-c | ✅ **로드맵 끝** — P0 화면 인상 · P3 엔진 움직임 · P4 연출 · 헤딩 · 락 음악 · **드리블 터치 간격(32차, G-75)** · **리플레이 측면 컷 확인(32차 — 카메라 좌표·캡처)** · 봇 120판(30차). P2 캐릭터 전면은 실사 삭제(G-49)로 없어졌다 | ✅ |
| 1 | 2차 피드백 반영 | 사용자가 "됐다" |
...
| 항목 | 보류 사유 + 재개 조건 |
|---|---|
| **재검증 WARN** (`npm run audit`) — 스로인 성공 43~49%(`npm run throwin` 가로채기 31%, 14차 24%) · 마커 54%(목표 60) · 시선 82%(목표 85) · 크로스 **수**가 판당 0.5번(질은 32차에 올렸다 — 1초 뒤 박스 안 1.33 · 2초 안 슛 33%. 봇이 스루 패스 위주라 사이드로 가는 일 자체가 드물다) · "깊은 사이드 소유 때 박스 안 동료" 0.6(이 지표는 압박받는 짧은 순간을 재서 크로스 질을 못 본다 — 크로스 줄을 본다) | 표본이 작거나 목표에 근접. 사용자 체감 제보가 오면 그때 |
...
- **(31차) "같은 키 두 번"은 날아가는 공을 바꾼다** — 킥이 떼는 순간 나가는 게임에서 기다리면 모든 패스가 늦어진다. 공이 떠난 뒤 9틱 안이면 1~2 m 앞이라 한 동작으로 보인다(`ball.followTap`).
- **(31차) 새 조작을 넣기 전에 그 조작이 기대는 규칙이 있는지 본다** — 벽 점프 앞에 "벽이 슛을 막나", 칩 앞에 "칩이 키퍼를 넘나". 둘 다 아니었다.
- **(31차) 선수를 치워 둔 상황(연습)은 11 대 11 이 가리던 구멍을 드러낸다** — 상대 둘 미만의 오프사이드 라인, PK 때 끌려오는 퇴장 선수. "순간이동"처럼 보이면 먼저 세트피스 배치(반칙 판정)를 의심한다.
- **(31차 둘째) "AI 가 성공하는 시범"은 녹화가 아니라 결정론으로** — 화면 없이 시드를 돌려 성공한 판을 찾고 같은 입력으로 다시 돌리면 똑같이 성공한다. 스쿼드가 바뀌어도 그 자리에서 다시 찾으면 된다.
- **(31차 둘째) 대본이 계속 실패하면 대본보다 엔진을 의심한다** — 여섯 개 중 넷이 엔진 구멍이었다(공중볼 도착점 · 벽 헤딩 · 슬라이딩 겨냥 · 러너 앞 공간). 추적 로그(틱마다 공·선수 좌표)를 한 번 찍으면 바로 보인다.
- **(31차) 계측을 백그라운드로 여러 번 띄우지 않는다** — `&` 로 띄운 것과 `run_in_background` 가 같은 파일에 동시에 썼다. 코드를 바꾼 뒤엔 이전 계측을 멈추고 하나만 다시.
