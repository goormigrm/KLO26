...
> **9/15 (16~24차)**: FC 온라인 벤치마크 계획서 → **실사 캐릭터 파이프라인**(glTF 스킨드 메시, `npm run char:build`, 사용자가 받은 Mixamo Ch38 적용) → P0 화면 인상 → P3 엔진 움직임 → 헤딩 → 락 음악 2곡 → P4 연출 → 저녁 제보 11건(GK 조작·시선·세트피스 배치·마킹·스루·크로스). **12번 팀·개인 전술은 다음 세션**(아래 0-d).
...
| 마지막 커밋 | 15차 중간 (패스 방향·D 압박·전력질주 모션·GK 다이브 회복) — `git log -1`. 그 앞 `0112eb0` 14차 |
...
| 테스트 | `npm test` — **21 파일 · 141개**(140 통과 · 1 skip — D 압박 세팅 미완) |
...
### [미반영] — 15차 중간 (시간이 끝나 멈춤)

| 무엇 | 상태 | 어떻게 잇나 |
|---|---|---|
| D 압박 테스트 `tests/feedback_0911b.test.ts` "소유자 3 m 안까지" | `it.skip` — 세팅에서 상대 조작 선수가 서 있게 하려고 두 팀을 사람으로 했는데도 3 m 안까지 못 감 | 60틱 동안 `me.x`·`ball.owner`·`opp` 위치를 찍어 볼 것(지운 `tools/_dbg_press.ts` 는 킥오프 S 잔여 버튼으로 상대가 패스해 버린 것까지 잡았다 — 빈 입력 2틱은 넣었다). 통과하면 `it` 로 |
| 브라우저 검증 | 안 함 | 혼자 하기 → E+방향키 홀드 스크린샷(팔 굽힘·상체 기울기) → 프리킥 슛 뒤 GK `action 3`(FALLEN) 0.5초 확인 |
| DESIGN 3.1 (D 압박·S 방향 기억) · 4.5 (패스 후보 점수) · 4.7 (다이브 뒤 FALLEN) · 7장(전력질주 모션) · 플레이 가이드 키 표 | 미반영 — CHANGELOG·DECISIONS G-43~46 에만 있다 | 14차 4.8d 처럼 4.8e 절로 묶어 적고 표 셀 갱신 |
| 봇 30판 파울 13 → 15 · 골 3.4 → 4.0 | 잡음인지 GK FALLEN 리바운드 때문인지 미확인 | `npm run balance -- 120` |
...
| 0-d | **팀 전술 · 개인 전술 (사용자 12번, 영상 "FC온라인 잘하는 법, 세팅부터 전개까지" — 강준호)** — 지금은 프리셋 3개(수비·균형·공격 = line·press·width·mentality). 설계: 팀 전술 = 수비 라인·압박·폭·템포·빌드업(짧게/길게)·공격 폭·수비 스타일(대인/지역), 개인 전술 = 선수별 역할(공격수 침투형/포스트형 · MF 박스투박스/홀딩 · 풀백 오버랩/스테이 · 윙 안으로/밖으로) → `Squad` 에 저장(온라인은 스쿼드 데이터로 전달), 스쿼드 전술판에서 칩 클릭 → 개인 전술, 로비/스쿼드에 팀 전술 슬라이더, 경기 중 `[ ]` 프리셋은 유지. sim 은 `ai.ts` 앵커·침투·마킹·패스 선호에 역할 계수 | 스쿼드 화면에 전술 탭 · 저장/불러오기 · `tests/tactics.test.ts` |
